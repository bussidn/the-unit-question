...
describe('PaymentService', () => {
  it('p3_processPayment_returnsSuccess_whenGatewaySucceeds', () => {
  ...
  it('p3_processPayment_returnsFailure_forZeroAmount_withoutCallingGateway', () => {
    const paymentGateway = { process: vi.fn(), refund: vi.fn() } as unknown as PaymentGateway;
    const paymentService = new GatewayPaymentService(paymentGateway);
  ...
  it('p3_processPayment_returnsFailure_whenGatewayFails', () => {
    const paymentGateway = { process: vi.fn(), refund: vi.fn() } as unknown as PaymentGateway;
    const paymentService = new GatewayPaymentService(paymentGateway);
  ...
  it('p3_refundPayment_delegatesToGateway', () => {
    const paymentGateway = { process: vi.fn(), refund: vi.fn() } as unknown as PaymentGateway;
    const paymentService = new GatewayPaymentService(paymentGateway);
