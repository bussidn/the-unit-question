...
**NSubstitute is gone entirely.** Every dependency is a real implementation backed by in-memory data:

```csharp
_orderService = new OrderService(
    _orderRepository,                              // InMemoryOrderRepository
    new RepositoryStockService(_stockRepository),  // real logic, in-memory data
    new PricingService(),                          // real — same as Part 3
    new GatewayPaymentService(_paymentGateway),    // real logic, in-memory gateway
    new GatewayShippingService(_shippingGateway)   // real logic, in-memory gateway
);
```

No `Substitute.For<>()`. No `.Returns()`. No `.Received()`. Tests describe business scenarios with real inputs and check real outcomes.

Have a look at the existing `OrderServiceTest` (`Part4.Behavior.Tests/Service/`) before you start.
...
💡 Open `Part4.Behavior/` as a **separate IDE project** to avoid editing the wrong `OrderService`.
...
- Wire real implementations by hand in the constructor
- Use `InMemoryStockRepository` and in-memory gateways
- Assert on real behavior instead of mock interactions

`InMemoryOrderRepository`, `InMemoryStockRepository`, `InMemoryDiscountCodeRepository`, and the in-memory gateways are available in `Helper/`.

`DiscountCodeService` is provided with its tests. It exposes two methods:

- **`CheckDiscountCode(customerId, discountCode)`** — returns `true` if the code has **already** been used by this customer
...
**1.2.** Add `DiscountCodeService` as a dependency. In `PlaceOrder`: if a discount code is present and **already used**, reject the order.
...
- Use `InMemoryDiscountCodeRepository` (in `Helper/`) to set up discount code state
...
No substitutes at all. `PlaceOrder` runs the entire chain — validation, pricing, stock, payment, shipping, persistence — with real implementations. The unit is **the use case**. Are these still "unit tests"?
